...
public class XmlMarketDao implements IMarketDao {
    ...
    private void init() {
	try {
	    if (!new File("build/resource").exists()) {
		new File("build/resource").mkdir();
	    }
	...
	try {
		...
		if (object.getClass().getDeclaredField("id").getInt(object) == fieldObject.getClass().getDeclaredField("id").getInt(fieldObject)) {
		    iter.remove();
		    break;
		}
	    }
