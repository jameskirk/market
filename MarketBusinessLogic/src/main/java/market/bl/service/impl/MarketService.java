...
public class MarketService implements IMarketOrderService, IMarketTaskService {
	...
	if (order.userOrderList == null || order.userOrderList.size() > 1 || (orderForAdmin = order.userOrderList.get(0)) == null) {
	...
	if (task.taskName == TaskName.REQUEST_PRICE_LIST) {
		...
		// no admin
		if (task.taskName == TaskName.REQUEST_PRICE_LIST) {
		    throw new MarketException("task " + TaskName.REQUEST_PRICE_LIST + " can be resumed only by admin");
		}
	    }
